...
import { User, Mail, Lock, UserPlus, Sparkles, ShieldCheck, UtensilsCrossed } from "lucide-react";
...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("https://mr-wari-backend-production.up.railway.app/api/auth/register", {
        method: "POST",
      ...
      >
        ...
        <div className="text-center mb-8 space-y-2">
          ...
          <p className="text-creamDim font-poppins text-xs uppercase tracking-widest pt-1">
            Join the Mister Wari Foodie Family
          </p>
        </div>
        ...
        <div className="p-8 rounded-3xl bg-bgPanel2/95 border border-line shadow-2xl space-y-6">
          ...
          {success && (
            <div className="p-3.5 rounded-2xl border border-emerald-500/40 bg-emerald-500/10 text-xs text-emerald-300 font-poppins">
              {success}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              <UserPlus className="w-4 h-4 text-[#14110d]" />
          ...
          <div className="pt-2 text-center border-t border-line/60">
            <p className="text-xs text-creamDim font-poppins">
              ...
              <Link to="/login" className="text-gold font-bold hover:underline ml-1">
                Login here
              </Link>
            </p>
          </div>
